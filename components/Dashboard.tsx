...
const Dashboard: React.FC<DashboardProps> = ({ responses }) => {
  ...
  const resetAllResponses = async () => {
    ...
    try {
      ...
      // Admin kontrolü API tarafında yapılacağı varsayımıyla
      ...
      if (!res.ok) {
        ...
        alert(
          'Silme başarısız.\n\n' +
            '1) Dashboard’u admin linkiyle açtığından emin ol\n' +
            '2) API tarafında DELETE endpoint/admin kontrolü var mı kontrol et\n\n' +
            (txt ? `Detay: ${txt}` : '')
        );
        return;
      }
  ...
  const calculateStats = () => {
    ...
    responses.forEach((r) => {
      Object.entries(r.scores).forEach(([id, score]) => {
        if (id in totals) totals[id] += Number(score);
      });
    });
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto px-4 py-8">
      {/* ✅ SIFIRLA BUTONU: HER ZAMAN GÖRÜNSÜN */}
      <div className="flex justify-end">
        ...
        >
          {isResetting ? 'Sıfırlanıyor...' : 'Tüm Kayıtları Sıfırla'}
        </button>
      </div>

      {/* Veri yoksa kısa mesaj göster */}
      {!stats ? (
        ...
        <>
                  ...
                  <BarChart data={stats.averages} layout="vertical">
                    ...
                    <Bar dataKey="A" radius={[0, 4, 4, 0]}>
                      {stats.averages.map((entry, index) => (
                        <Cell
                          key={`cell-${index}`}
                          fill={entry.A > 4 ? '#10b981' : entry.A > 3 ? '#4f46e5' : '#f59e0b'}
                        />
                      ))}
                    </Bar>
                  </BarChart>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {(['stopDoing', 'startDoing', 'keepDoing'] as const).map((type) => (
              <div key={type} className="bg-white p-6 rounded-3xl shadow border border-gray-100">
                ...
                <ul className="space-y-3">
                  {responses
                    .map((r) => r.feedback?.[type])
                    .filter(Boolean)
                    .slice(0, 5)
                    .map((text, i) => (
                      <li
                        key={i}
                        className="text-sm text-gray-600 bg-gray-50 p-3 rounded-lg border border-gray-100 italic"
                      >
                        "{text}"
                      </li>
                    ))}
